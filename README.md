...
- 입력
  - [ ] 게임을 계속할지 여부를 입력받는다.
    - [ ] 게임을 계속할지 여부를 물어보는 프롬프트를 출력한다.
    - [ ] [예외처리] 플레이어의 입력이 Y 또는 N이 아닐 경우 예외를 발생시킨다.
    - [ ] 예외가 발생한 경우 메시지 출력 후 재입력 받는다. 
...
- 로직
  ...
  - [ ] 승패를 결정한다.
    - [ ] 더 큰 값을 갖는 사람이 승리한다.
    - [ ] 같은 값이라면 서로 비기게 된다.
  - [ ] 입력 값에 따라 게임을 계속할지 결정한다.
    - [ ] 플레이어가 Y를 입력할 경우 한 게임 더 진행한다.
    - [ ] 플레이어가 N을 입력할 경우 게임을 종료한다.
- 출력
  - [ ] 최초 게임을 시작하면 게임의 타이틀을 표시한다.
